...
describe("SeasonOne", function () {
  ...
  before(async function () {
    ...
    CloakLib = (await ethers.getContractFactory("Cloak")) as Cloak__factory
    ...
    cloak = await CloakLib.attach(deployData['CloakLib'])
  ...
  describe("upon burnShardForPower", () => {
    ...
    it("requires that the user holds a shard", async ()=> {
      await expect(seasonOne.burnShardForPower(1,1)).to.be.revertedWith("E-000-007")
    })

    it("requires that the user specifies a non-zero shard amount", async ()=> {
      await expect(seasonOne.connect(userA).burnShardForPower(1,0)).to.be.revertedWith("E-000-006")
    })
    ...
    it("requires that the user holds the amount of shard they're trying to burn", async ()=> {
      await expect(seasonOne.connect(userA).burnShardForPower(1,1000000)).to.be.revertedWith("E-000-007")
    })
    ...
    it("successfully burns shard", async ()=> {
      ...
      let powerBefore = await seekers.getPowerById(id)
      await seasonOne.connect(userA).burnShardForPower(id,1)
      let shardBalAfter = await seasonOne.balanceOf(userA.address, 1)
      let powerAfter = await seekers.getPowerById(id)
      expect(shardBalBefore).to.be.equal(shardBalAfter.add(1))
      expect(powerAfter).to.be.greaterThan(powerBefore)
    })
  })
  ...
  describe("upon stakeShardForCloin", () => {
    ...
    it("requires that the user holds a shard", async ()=> {
      await expect(seasonOne.stakeShardForCloin(1)).to.be.revertedWith("E-000-007")
    })

    it("requires that the user specifies a non-zero shard amount", async ()=> {
      await expect(seasonOne.connect(userA).stakeShardForCloin(0)).to.be.revertedWith("E-000-006")
    })

    it("requires that the user holds the amount of shard they're trying to stake", async ()=> {
      await expect(seasonOne.connect(userA).stakeShardForCloin(1000000)).to.be.revertedWith("E-000-007")
    })
  ...
  describe("upon burn shard for fragment", () => {
    ...
    it("requires the user to specify a non zero shard amount", async () => {
      await expect(seasonOne.connect(userA).burnShardForFragments(0)).to.be.revertedWith("E-000-006")
    })

    it("requires that the user specify an even multiple of the shard-to-fragment conversation rate", async () =>  { 
      await expect(seasonOne.connect(userA).burnShardForFragments(SHARDTOFRAGMENT - 1)).to.be.revertedWith("E-000-008")
    })
